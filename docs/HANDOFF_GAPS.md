# 【済】実装不足の是正: 引き継ぎ指示書（第1次）

> **ステータス: 完了（クローズ済み）**
>
> 本文書のタスク A〜G はすべて実装・検証・コミットが完了した。**この文書はもう作業指示ではなく、
> 完了記録である。** 新規の作業に着手する場合は後続の
> **[`docs/HANDOFF_GAPS_2.md`](HANDOFF_GAPS_2.md)（第2次・未着手）** を参照すること。
>
> 各機能の仕様詳細・数式・Cソース引用は `docs/PORT_NOTES.md` にも整理されている。
...
上記の実装完了後にレビューを行い、以下2件を修正した。

- ✅ **プレイヤー弾の判定ミスによる進行不能バグ** (`3d63064`)
  `main.js` が「`isEnemy` 属性を持たない＝Saricの発射した弾」と判定していたが、この属性は
  看板・店主・アイテム箱など配置エンティティ586体中110体にも無い。結果、**取得可能アイテム
  31件すべて・店5軒すべて・会話NPC99件中98件**が機能せず、開始画面の短剣が取れないため
  ゲームが最初から進行不能だった。明示フラグ `firedByPlayer` に置き換え、命中時の自壊も
  `isMissile` 保持時のみに限定（`EnemyCollision.c:727-730`）。
- ✅ **音楽が一切鳴らない不具合** (`578a484`)
  `audio.js` が存在しない `player.load()` を呼んでおり（実体は `loadBuffer()`）、例外が
  `catch` に握りつぶされていた。加えてロード後の `play()` / `setVolume()` 呼び出しも
  欠落していた。両方を修正。

> この2件から得た再発防止の教訓（`catch` に握りつぶされる例外、
> 「実装済み・呼び出し元ゼロ」の見落とし、HTTPサーバのキャッシュ）は
> `docs/HANDOFF_GAPS_2.md` の 0-1 節にまとめてある。
