...
//  - the channel array is built from mod.channels instead of being four
//    Channel objects fixed at construction time
//  - output is stereo with Amiga-style panning, and the sum is scaled by the
//    channel count so a 6CHN module doesn't run hotter than a 4-channel one
//  - the row message carries pattern, speed and bpm, so a UI can display them
//  - an optional level message drives per-channel meters
...
// How often the meter levels go out, in output samples. Roughly 20ms at 48k,
// which is faster than anyone can see and cheap enough not to matter.
...
class Channel {
    constructor(worklet, index) {
        ...
        // Amiga hardware alternates LRRL across the channels; anything past
        // the first four repeats the pattern.
        const left = (index % 4 === 0 || index % 4 === 3);
        const pan = left ? -0.6 : 0.6;
    ...
    // LOCAL CHANGE: silence a channel outright, for stop and for seeking.
    silence() {
        this.instrument = null;
        this.period = 0;
        this.currentPeriod = 0;
    ...
    play(note) {
        ...
        if (note.instrument) {
            // LOCAL CHANGE: a MOD holds 31 instruments, but modules carry
            // stray numbers past that - track_128 has a 64 at pattern 1 row 0.
            // ProTracker ignores them; reading past the array throws, and a
            // worklet that throws is retired for good, so the music would just
            // stop there and never come back.
            const instrument = this.worklet.mod.instruments[note.instrument];
            if (instrument) {
                this.setInstrument = instrument;
                this.setVolume = instrument.volume;
            }
        }
...
class ModPlayerWorklet extends AudioWorkletProcessor {
    ...
    play(mod, sampleRate) {
        ...
        // LOCAL CHANGE: one Channel per channel the module actually has, and a
        // mix scale that keeps the peak sum the same whatever that count is.
        const count = mod.channels || 4;
        this.channels = [];
        for (let i = 0; i < count; ++i) {
            this.channels.push(new Channel(this, i + 1));
        }
        this.levelPeaks = new Array(count).fill(0);
        this.mixScale = 4 / count;
    ...
    setRow(position, row) {
        ...
        this.outputsUntilNextTick = 0;
        this.patternBreak = false;
        // LOCAL CHANGE: drop whatever was sounding, or a held note drags across
        // the jump.
    ...
    nextRow() {
        ...
        const row = pattern && pattern.rows[this.rowIndex];
        if (!row) return;
        ...
        if (this.publishRow) {
            this.port.postMessage({
                ...
                // LOCAL CHANGE: the extra fields a transport display needs.
    ...
    // LOCAL CHANGE: stereo. Returns nothing - it writes the pair through the
    // out array to keep the per-sample path allocation free.
    nextOutput(out) {
        out[0] = 0.0;
        out[1] = 0.0;
        ...
        for (let i = 0; i < this.channels.length; ++i) {
            ...
            const value = channel.nextOutput();
            left += value * channel.gainLeft;
            right += value * channel.gainRight;
            if (this.publishLevels) {
                const level = value < 0 ? -value : value;
                if (level > this.levelPeaks[i]) this.levelPeaks[i] = level;
            }
        }
